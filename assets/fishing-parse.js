...
const FISHING_PATH_RE = /\/(fish|fishing)(\/|$)|play_cards|move_focus/i;
...
function hasStrongFishingSignals(response) {
  if (response == null || typeof response !== "object") return false;
  const blob = safeJsonKeys(response);
  if (
    /\bfishing\b|\bfishId\b|\bfish_id\b|\bcatchMeter\b|\bfishCell\b|\bfintuition\b|\bplay_cards\b|\bmove_focus_point\b|\bbobber\b|\bfocusPoint\b|\bFISH_CID\b|\bFISHING_CID\b/i.test(
      blob,
    )
  ) {
    return true;
...
export function looksLikeFishingPayload(response, requestAction, url) {
  ...
  return hasStrongFishingSignals(response);
}
...
export function extractFishPosition(response) {
  ...
  const consider = (raw, source, score) => {
    const parsed = parseBoardPosition(raw);
  ...
  return { pos: best.pos, cell: best.cell, source: best.source, candidates: candidates.slice(0, 6), diagnostic: null };
}

export function extractBobberPosition(response) {
  ...
  candidates.sort((a, b) => b.score - a.score);
  const best = candidates[0];
  return { pos: best.pos, cell: best.cell, source: best.source, candidates: candidates.slice(0, 6) };
}
...
export function extractFocus(response) {
  ...
  walk(response, "", (key, value, path) => {
    ...
    if (!fishing) return;
    if (/^focus(current|now|left|remaining)?$/i.test(key) || /^currentfocus$/i.test(key)) {
    ...
    if (/focusmax|maxfocus/i.test(key)) {
      const n = asFinite(value);
      if (n != null) max = n;
    }
  });
  return { current, max, source, found: current != null };
}
...
export function normalizeSpellCard(raw, index = 0) {
  ...
  const addLocalCells = (list, kind) => {
    ...
    for (const item of arr) {
      ...
      const n = asInt(item);
      if (n != null) {
        const off = localCellToOffset(n >= 0 && n <= 8 ? n + 1 : n);
        if (off) pushOffset(target, off.dx, off.dy, kind);
      }
    }
  };
  addLocalCells(raw.hits ?? raw.hitCells ?? raw.hitBoxes ?? raw.blue ?? raw.tiles ?? raw.covered, "hit");
  addLocalCells(raw.crits ?? raw.critCells ?? raw.yellow ?? raw.gold ?? raw.critical, "crit");
  ingestPatternGrid(
    target,
    raw.grid ?? raw.pattern ?? raw.hitGrid ?? raw.cells ?? raw.board ?? raw.hitboxes ?? raw.layout,
  );
  if (hitOffsets.length + critOffsets.length === 0 && !raw.grid && !raw.pattern) return null;
...
export function extractFishingHand(response) {
  ...
  walk(response, "", (key, value, path) => {
    ...
    if (/fintuition|revealed|nextcell|nexttile|hintcell|predictedcell/i.test(key)) {
      const parsed = parseBoardPosition(value?.cell ?? value?.position ?? value);
      if (parsed.cell != null) revealedCell = parsed.cell;
    }
  });
  return { cards, mana, catchMeter, catchMax, revealedCell };
}
